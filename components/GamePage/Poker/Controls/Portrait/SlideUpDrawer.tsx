...
export function SlideUpDrawer() {
    ...
    const { players, gameState, playersToAway } = useGameData();
    ...
    // Disable if already pending away next hand or already away
    const currentPlayer = players.find((p: any) => p.player_id === userId);
    const isAwayDisabled = playersToAway.includes(userId) || currentPlayer?.action === Action.AWAY;
