...
export const PlayerHandBox = ({ player, isYou }: PlayerHandBoxProps) => {
    const bg =
        'linear-gradient(351.96deg,rgb(69, 0, 142) -20.9%, rgba(78, 35, 122, 0) 20%,rgb(136, 80, 183) 96%), #4B2B82';
    ...
    if (player?.hand_name) {
        handNames = Array.isArray(player.hand_name)
            ? player.hand_name
            : [player.hand_name];
    }
    const handDetails: (string | null | undefined)[] =
        player?.hand_details || [];
    ...
    const filteredHandPairs = handNames
        ...
        .filter((pair) => pair.name !== HandName.LAST_PLAYER_STANDING);

    if (filteredHandPairs.length > 0 && isYou) {
        const handTextArr = filteredHandPairs.map(({ name, detail }, idx) => {
            const base =
                handNameToText[name as keyof typeof handNameToText] ||
                String(name);
