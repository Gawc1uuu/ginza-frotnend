...
export const AvatarProgress = ({
    ...
    const size = isPortrait ? '6.5vmax' : '6.25vmin';
    const outerSize = isPortrait ? '7.1vmax' : '7.05vmin'; // Even larger for purple bar
        ...
        <>
            ...
            {extraTimeActivated && (
                <CircularProgress
                    value={100 * 0.58}
                    max={100}
                    size={isPortrait ? '6.9vmax' : '6.7vmin'}
                    thickness={isYourTurn ? '0.65rem' : '0.58rem'}
                    position="absolute"
                    zIndex={0}
                    top="50.1%"
                    left={isPortrait ? '48.1%' : '48.2%'}
                    borderRadius="100%"
                    style={{
                        transform:
                            'translate(-50%, -50%) scaleX(1) rotate(0deg)',
                    }}
