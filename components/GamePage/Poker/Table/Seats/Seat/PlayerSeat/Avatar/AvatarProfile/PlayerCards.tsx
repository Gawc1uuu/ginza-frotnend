...
export const PlayerCards = ({
    ...
    useEffect(() => {
        ...
        prevShowRef.current =
            player?.hand?.map((card: any) => card?.show) || [];
    }, [player?.hand]);

    const twoCardFrontTrans = [
        isPortrait
            ? 'rotate(-5deg) translateX(0.4vmax) translateY(-0.75vmax)'
            : 'rotate(-5deg) translateX(-0.4vmin) translateY(0.2vmin)',
        isPortrait
            ? 'rotate(5deg) translateX(2.3vmax) translateY(-1.32vmax)'
            : 'rotate(5deg) translateX(2.1vmin) translateY(-0.42vmin)',
    ];

    const twoCardBackTrans = [
        isPortrait
            ? 'rotate(-6deg) translateY(0.47vmax)'
            : 'rotate(-8deg) translateY(1.1vmin)',
        isPortrait
            ? 'rotate(6deg) translateX(2.25vmax) translateY(0.28vmax)'
            : 'rotate(7deg) translateX(2.75vmin) translateY(0.75vmin)',
    ];

    const fourCardFrontTrans = [
        isPortrait
            ? 'rotate(-10deg) translateX(-0.3525vmax) translateY(-1.05vmax)'
            : 'rotate(-10deg) translateX(-1.1vmin) translateY(-0.55vmin)',
        isPortrait
            ? 'rotate(-3deg) translateX(1.5vmax) translateY(-1.3vmax)'
            : 'rotate(-3deg) translateX(0.9vmin) translateY(-0.8vmin)',
        isPortrait
            ? 'rotate(3deg) translateX(3.5vmax) translateY(-1.7vmax)'
            : 'rotate(3deg) translateX(2.95vmin) translateY(-1.26vmin)',
        isPortrait
            ? 'rotate(10deg) translateX(5.05vmax) translateY(-2.45vmax)'
            : 'rotate(10deg) translateX(4.85vmin) translateY(-1.95vmin)',
    ];

    const fourCardBackTrans = [
        isPortrait
            ? 'rotate(-10deg) translateX(-0.15vmax) translateY(0.3vmax)'
            : 'rotate(-10deg) translateY(0.5vmin)',
        isPortrait
            ? 'rotate(-3deg) translateX(1.25vmax) translateY(0.25vmax)'
            : 'rotate(-3deg) translateX(1.5vmin) translateY(0.4vmin)',
        isPortrait
            ? 'rotate(3deg) translateX(2.5vmax) translateY(0.05vmax)'
            : 'rotate(3deg) translateX(3vmin) translateY(0.2vmin)',
        isPortrait
            ? 'rotate(10deg) translateX(3.75vmax) translateY(-0.3vmax)'
            : 'rotate(10deg) translateX(4.25vmin) translateY(-0.175vmin)',
    ];

    const cardTrans =
        (player?.hand?.length ?? 0) > 2
            ? fourCardFrontTrans
            : twoCardFrontTrans;
    const cardBackTrans =
                ...
                return (
                    ...
                    >
                        <Card
                            ...
                            cardIndex={index}
                            cornerSize={
                                isPortrait ? '1.2vmax' : cardCornerSizeLandscape
                            }
                            cornerValueSize={
                                isPortrait
                                    ? '1.6vmax'
                                    : cardCornerValueSizeLandscape
                            }
                            cornerSpacing={
                                isPortrait
                                    ? '-0.1vmax'
                                    : cardCornerSpacingLandscape
                            }
                            cornerMarginTop={
                                isPortrait
                                    ? '-0.2vmax'
                                    : cardCornerMarginTopLandscape
                            }
                            cornerValueSizeBottom={
                                isPortrait
                                    ? '1.5vmax'
                                    : cardCornerValueSizeBottomLandscape
                            }
                            shouldAnimate={false}
