...
const landscapeStyles: Record<PlayerPosition, Record<string, string>> = {
    ...
    right: { right: '-11vmin', top: '-5.1vmin' },
...
export const DealerChip = ({ player, isDealer }: DealerChipProps) => {
    ...
    if (isDealer) {
        return (
            ...
            >
                <Image
                    src="/icons/DealerChipIcon1.webp"
                    alt="Dealer Chip"
                    height={isPortrait ? '1.6vmax' : '2.03vmin'}
                    width={isPortrait ? '2.1vmax' : '2.75vmin'}
                    filter={isPortrait ? 'drop-shadow(0 0.3vmax 0.25vmax rgba(0, 0, 0, 0.35))' : 'drop-shadow(0 0.3vmin 0.25vmin rgba(0, 0, 0, 0.4))'}
                    userSelect="none"
                    draggable={false}
                />
            </Box>
