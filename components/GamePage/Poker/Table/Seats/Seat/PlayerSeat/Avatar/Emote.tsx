...
export const Emote = ({ player }: EmoteProps) => {
    ...
    const { data: emotes = [] } = useWSQuery<any[]>(
        WebsocketsQueries.getEmotes,
        {
            gameId,
        },
    );
    ...
    useEffect(() => {
        if (emotes.length > 0) {
            ...
            const recentEmotes = emotes
                .filter((em) => {
                    const timeDiff =
                        currentTime - new Date(em.createdAt).getTime();
            ...
            if (latestPlayerEmote) {
                ...
                const emoteTimestamp = new Date(
                    latestPlayerEmote.createdAt,
                ).getTime();
