...
export const ExtraTimerButton = ({
    isYou,
    isYourTurn,
}: ExtraTimerButtonProps) => {
    const isPortrait = useBreakpointValue({ base: true, lg: false, xl: false });
    const { gameId, extraTimeActivated, gameSettings, currentDecidingPlayer } =
        useGameData();
    ...
    useEffect(() => {
        ...
        isYourTurn,
        extraTimeActivated,
        user?.pokerPreferences?.autoActivateExtraTime,
        gameId,
        toast,
    ]);
