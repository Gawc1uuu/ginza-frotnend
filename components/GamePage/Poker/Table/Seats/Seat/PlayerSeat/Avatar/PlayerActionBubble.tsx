...
if (
    ...
    !document.getElementById('player-bubble-anim')
) {
...
export const PlayerActionBubble = ({
    ...
    useEffect(() => {
        ...
        // If bet_amount goes from nonzero to zero, trigger exit animation
            ...
            prevBetAmount.current &&
            prevBetAmount.current > 0n &&
            (!player.bet_amount || player.bet_amount === 0n)
        ) {
            setIsExiting(true);
            ...
            setTimeout(() => {
                if (bubbleRef.current) {
                    ...
                    setFlyStyle({
                        transition:
                            'transform 0.45s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.55s',
    ...
    const getBubbleVariant = (): BubbleVariant | null => {
        ...
        if (player.action === Action.CHECK) {
            return {
                ...
                content: 'CHECK',
                boxShadow:
                    '0 8px 16px rgba(0,0,0,0.45), inset 0 1px 1px rgba(255,255,255,0.08)',
            };
        }
        if ((player.bet_amount && player.bet_amount > 0n) || isExiting) {
            // Always show lastNonzeroBetDisplay during exit
            const display = isExiting
                ? lastNonzeroBetDisplay
                : betAmountDisplay;
            return {
            ...
            ((player.bet_amount && player.bet_amount > 0n) || isExiting) && (
                ...
                >
                    <HStack
                        ...
                        border="0.5px solid"
                        borderColor="rgba(219, 237, 255, 0.25)"
                        spacing="0.25rem"
                        pr="0.4rem"
                        pl="0.4rem"
                        width="100%"
    ...
    const containerBg =
        variant.content === 'CHECK'
            ? 'linear-gradient(180deg, #3B57A6 0%, #2E407A 100%)'
            : 'linear-gradient(180deg, #11181F 0%, #0B1116 100%)';

    const isCheck = variant.content === 'CHECK';
    ...
    return (
        <Container
            ...
            alignItems="center"
            justifyContent="center"
            sx={{ backgroundImage: containerBg }}
            backgroundColor={variant.backgroundColor}
            boxShadow={
                isCheck
                    ? '0 4px 10px rgba(24, 56, 120, 0.35)'
                    : variant.boxShadow
            }
            border={isCheck ? '1px solid' : '1px solid'}
            borderColor={
                isCheck
                    ? 'rgba(120, 160, 230, 0.85)'
        ...
        >
            <Text
                variant="bold"
                fontSize={isPortrait ? '1.5vmax' : '1.65vmin'}
                lineHeight={isPortrait ? '0.875vmax' : '1.125vmin'}
                color="white"
            >
                {variant.content}
