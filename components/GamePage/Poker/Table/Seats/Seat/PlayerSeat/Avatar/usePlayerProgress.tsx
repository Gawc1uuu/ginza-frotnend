        ...
        const calculateProgress = (timestamp: number) => {
            if (timestamp - lastUpdateTime.current >= 50) {
                ...
                if (extraTimeActivated) {
                    ...
                    const regularTimeRemaining = Math.max(
                        0,
                    ...
                    newProgress =
                        regularTimeRemaining > 0
                            ? (regularTimeRemaining / turnTime) * 100
                            : 0;

                    // Purple bar animates only during extra time
                    if (timeRemaining <= extraTime) {
                        newExtraTimeProgress =
                            (extraTimeRemaining / extraTime) * 100;
                    } else {
                    ...
                    newProgress = Math.max(
                        0,
                        Math.min(100, progressValue - 0.5),
                    );
                    newExtraTimeProgress = 100;
                }
