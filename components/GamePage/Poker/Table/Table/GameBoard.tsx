...
export const StartButton = () => {
    ...
    const handleStartError = (error: unknown) => {
        const errorMessage = (error as any)?.detail ?? 'An unknown error occurred';
        toast({
    ...
    const activePlayers = players?.filter((p: any) => p.action !== Action.AWAY)?.length ?? 0;
    const isDisabled = activePlayers < 2;
    ...
    return (
        <APIButton
            ...
            disabled={isDisabled}
            bgGradient="linear(to-b, brand.gray30 10%, brand.gray70 95%)"
            border="1px solid"
        ...
        >
            <Text
                ...
                color={isDisabled ? 'brand.gray20' : 'brand.accentWhite'}
            >
                Start Game
            </Text>
        </APIButton>
