...
export interface CreateTableFormValues {
    ...
    visibility: number; // 0 = ADMIN, 1 = PRIVATE, 2 = PUBLIC
...
export const createTableSchema = object<CreateTableFormValues>({
    ...
    minBuyIn: number()
        ...
        .test(
            'min-lte-max',
            'Min buy-in must be <= max buy-in',
            function (value) {
                const { maxBuyIn } = this.parent;
                if (typeof value !== 'number' || typeof maxBuyIn !== 'number')
                    return true;
    ...
    bombPotFrequency: number()
        ...
        .required(),
    visibility: number().oneOf([1, 2]).required(),
    doubleBoard: mixed<'always' | 'bomb_pot' | 'off'>()
        .oneOf(['always', 'bomb_pot', 'off'])
        .required(),
    mandatoryStraddle: boolean().required(),
});
...
export const defaultValues: CreateTableFormValues = {
    ...
    bombPotBB: 0,
    bombPotFrequency: 0,
    visibility: 1, // Default to PRIVATE
    doubleBoard: 'off',
    mandatoryStraddle: false,
};
