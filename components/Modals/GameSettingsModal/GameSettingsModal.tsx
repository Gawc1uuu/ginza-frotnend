...
const GameSettings: React.FC<GameSettingsModalProps> = ({
    ...
    const [doubleBoard, setDoubleBoard] = useState<
        'always' | 'bomb_pot' | 'off'
    >('off');
    const [mandatoryStraddle, setMandatoryStraddle] = useState<'yes' | 'no'>(
        'no',
    );
    ...
    useEffect(() => {
        setBombPotBB(
            gameSettings?.bomb_pot_bb > 0
                ? gameSettings.bomb_pot_bb.toString()
                : '',
        );
        setBombPotFrequency(
            gameSettings?.bomb_pot_frequency > 0
                ? gameSettings.bomb_pot_frequency.toString()
                : '',
        );
        ...
        // Map backend enum to dropdown value
        if (gameSettings?.double_board === 0) setDoubleBoard('always');
        else if (gameSettings?.double_board === 2) setDoubleBoard('bomb_pot');
        else setDoubleBoard('off');

        setMandatoryStraddle(
            (gameSettings as any)?.mandatory_straddle ? 'yes' : 'no',
        );
    }, [
        ...
        gameSettings?.double_board,
        (gameSettings as any)?.mandatory_straddle,
    ]);

    const hasChanges =
        (bombPotBB !== '' &&
            bombPotBB !== gameSettings?.bomb_pot_bb?.toString()) ||
        (bombPotFrequency !== '' &&
            bombPotFrequency !==
                gameSettings?.bomb_pot_frequency?.toString()) ||
        ...
        (ante !== '' &&
            gameSettings?.ante_value !== undefined &&
            gameSettings?.big_blind_value !== undefined &&
            ante !==
                (
                    gameSettings.ante_value / gameSettings.big_blind_value
                ).toFixed(1)) ||
        (ante === '' && gameSettings?.ante_value !== 0) ||
        (doubleBoard === 'always' && gameSettings?.double_board !== 0) ||
        (doubleBoard === 'bomb_pot' && gameSettings?.double_board !== 2) ||
        (doubleBoard === 'off' && gameSettings?.double_board !== 1) ||
        (mandatoryStraddle === 'yes') !==
            !!(gameSettings as any)?.mandatory_straddle;
    ...
    const handleSettingsChange = async () => {
        ...
        try {
            ...
            if (
                bombPotFrequency !== '' &&
                (isNaN(newFreq) || newFreq < 1 || newFreq > 30)
            ) {
            ...
            const anteInMicroDollars =
                newAnte * (gameSettings.big_blind_value || 0);

            // Map dropdown value to backend enum
            const doubleBoardEnum =
                doubleBoard === 'always'
                    ? 0
                    : doubleBoard === 'bomb_pot'
                      ? 2
                      : 1;
            ...
            const updateSettings: any = {
                ...
                mandatory_straddle: mandatoryStraddle === 'yes',
            };
            updateSettings.double_board = doubleBoardEnum;
            ...
            if (gameState === 0) {
                ...
                toast({
                    ...
                    description:
                        'New Settings will be applied at the start of the next hand.',
                    status: 'info',
                    duration: 5000,
                    isClosable: true,
                });
            ...
            content={
                <VStack spacing={4} w="100%">
                    ...
                    <HostSettingsStep
                        ...
                        setDoubleBoard={setDoubleBoard}
                        mandatoryStraddle={mandatoryStraddle}
                        setMandatoryStraddle={setMandatoryStraddle}
                    />
                </VStack>
            }
            primaryButton={
