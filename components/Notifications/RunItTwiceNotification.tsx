...
const VotePrompt = ({ onVoteSubmitted }: VotePromptProps) => {
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            setProgress(
                Math.max(0, Math.min(100, (timeLeft / TIMER_FULL) * 100)),
            );
        };
        update();
            ...
            {isPlayer && !isSpectator && !isAway && !isFolded && (
                <HStack w="100%" spacing={2}>
                    <Button
                        ...
                        bg={
                            submittedVote !== null
                                ? submittedVote === false
                                    ...
                                    : 'gray.600'
                                : alreadyVoted &&
                                    dealChoiceVotes[myPlayerId] === 2
                                  ? 'gray.600'
                                  : undefined
                        }
                        _hover={
                            submittedVote !== null ||
                            (alreadyVoted && dealChoiceVotes[myPlayerId])
                                ? { bg: 'gray.600' }
                                : undefined
                        }
                        _active={
                        ...
                        cursor={
                            submittedVote !== null || alreadyVoted
                                ? 'not-allowed'
                                : undefined
                        }
                    >
