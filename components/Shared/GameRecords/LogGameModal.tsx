...
export const LogGameModal: React.FC<LogGameModalProps> = ({
    ...
    // Query for game data
    const { data: gameData } = useWSQuery<any>(
        WebsocketsQueries.getGameData,
        {
            gameId: gameId,
        },
    ...
    const gameCreationDate = gameData?.gameData?.game_start_time
        ? new Date(gameData.gameData.game_start_time)
        : null;
    const june20_2025 = new Date('2025-06-20');
    const hasNoLogs =
        !gameData?.gameData?.nonce ||
        gameData.gameData.nonce < 2 ||
        (gameCreationDate && gameCreationDate <= june20_2025);
    ...
    const { data: handHistoryLogs } = useWSQuery<GameLogDto[]>(
        WebsocketsQueries.getHandHistoryLogsPublic,
        {
            gameId: gameId,
            handNumber: submittedHandNumber,
        },
        Boolean(submittedHandNumber && gameId),
    );
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (
            isNaN(handNum) ||
            handNum < 1 ||
            handNum > maxHandNumber ||
            !Number.isInteger(handNum)
        ) {
            setError(
                `Hand number must be an integer between 1 and ${maxHandNumber}`,
            );
            setSubmittedHandNumber('');
            return;
        }
                        ...
                        {!isLoading && hasNoLogs && (
                            ...
                            >
                                ...
                                <Text color="red.100" fontSize="sm" mt={2}>
                                    This game has no hand history logs to
                                    display.
                                </Text>
                            </Box>
                        )}
                            ...
                            >
                                ...
                                handHistoryLogs === undefined ? (
                                  ...
                                  handHistoryLogs.length > 0 ? (
                                    ...
                                    <Text color="gray.400">
                                        No hand history found for hand #
                                        {submittedHandNumber}.
                                    </Text>
                                ) : (
                                    <Text color="gray.400">
                                        Enter a hand number to view the log.
                                    </Text>
                                )}
                            </Box>
                        )}
