...
const SoundManager: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isNewTurn) {
            const prevP = players.find(
                (p: any) => p.player_id === lastPlayerId,
            );
    ...
    useEffect(() => {
        ...
        if (
            !runItTwice?.second_board ||
            runItTwice.second_board.length <= nSecondBoardRevealed
        )
            return;
    ...
    useEffect(() => {
        ...
        const checkAndPlayTimerAudio = () => {
            ...
            if (
                timeLeft <= fiveSecondsLeftThreshold &&
                timeLeft > 0 &&
                !timerSoundPlayed
            ) {
